...
fn test_init_success() {
    ...
    let ix = init_ix(program_id, &payer.pubkey(), state_key, vault_key, &sequencer.pubkey(), state_bump, vault_bump);
    let result = send(&mut svm, ix, &payer, &[&payer]);
    ...
    assert_eq!(&d[STATE_SEQ_PUBKEY..STATE_SEQ_PUBKEY + 32], sequencer.pubkey().as_ref());
...
fn test_init_already_initialized() {
    ...
    let ix2 = init_ix(program_id, &payer.pubkey(), state_key, vault_key, &sequencer.pubkey(), state_bump, vault_bump);
...
fn test_init_bad_pda() {
    ...
    svm.set_account(wrong_state.pubkey(), Account {
        lamports: 1_141_440,
        data: vec![0u8; STATE_SZ],
        owner: program_id,
        executable: false,
    ...
    let ix = init_ix(program_id, &payer.pubkey(), wrong_state.pubkey(), vault_key, &sequencer.pubkey(), state_bump, vault_bump);
    let result = send(&mut svm, ix, &payer, &[&payer]);
